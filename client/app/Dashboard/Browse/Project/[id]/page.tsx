...
import { 
  ...
  FileSpreadsheet, Paperclip, MessageSquare, Eye, Edit3, Trash2 
} from 'lucide-react';
...
export default function PurchaseOrderSection({ projectId: propProjectId }: { projectId: string }) {
  ...
  const [showEditForm, setShowEditForm] = useState(false); // 🚀 編輯彈窗控制
  const [uploadingId, setUploadingId] = useState<string | null>(null);
  const [selectedPoForRemark, setSelectedPoForRemark] = useState<PO | null>(null);
  const [remarkContent, setRemarkContent] = useState("");
  const [editingPo, setEditingPo] = useState<PO | null>(null); // 🚀 當前正在編輯的 PO
  ...
  const handleUpdatePo = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 串接後端更新 API：/api/purchaseOrder/:id
      await api.patch(`/api/purchaseOrder/${editingPo.id}`, payload);
  ...
  const handleDeletePo = async (poId: string, poNumber: string) => {
    if (!confirm(`⚠️ 警告：確定要刪除採購單 [${poNumber}] 嗎？此動作將連帶受影響之資料，且無法復原。`)) return;
    try {
      // 串接後端刪除 API：/api/purchaseOrder/:id
      await api.delete(`/api/purchaseOrder/${poId}`);
      fetchPOs();
    } catch (err) {
    ...
    <div className="mt-6 border-t border-slate-800 pt-6">
      ...
      <div className="space-y-3">
        {loading ? (
          ...
          pos.map(po => (
            <div key={po.id} className="bg-slate-900/60 border border-slate-800 p-4 group rounded-sm relative">
                ...
                <div className="text-right flex flex-col items-end gap-2">
                  <div className="flex items-center gap-2">
                    ...
                    <button 
                      ...
                      className="opacity-0 group-hover:opacity-100 p-1.5 rounded-sm hover:bg-red-500/10 text-slate-600 hover:text-red-400 transition-all"
                      title="刪除採購單"
                    >
                      <Trash2 size={13} />
                    </button>
                    ...
                    <button 
                      ...
                      className="opacity-0 group-hover:opacity-100 p-1.5 rounded-sm hover:bg-yellow-500/10 text-slate-600 hover:text-yellow-400 transition-all"
                      title="編輯採購單"
                    >
                      <Edit3 size={13} />
                    </button>
          ...
          <div className="bg-slate-900 border border-blue-500/50 p-8 w-full max-w-md shadow-[0_0_50px_rgba(37,99,235,0.2)]">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input 
                  required value={formData.vendor}
                  className="w-full bg-slate-800 border border-slate-700 p-2 text-white outline-none focus:border-blue-500"
      ...
      {showEditForm && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-slate-900 border border-yellow-500/50 p-8 w-full max-w-md shadow-[0_0_50px_rgba(234,179,8,0.2)]">
            <h2 className="text-xl font-bold text-white mb-2 tracking-tighter italic uppercase">Edit_Purchase_Order</h2>
            <p className="text-[9px] font-mono text-yellow-500/80 mb-6">UPDATE_TARGET // {editingPo?.poNumber}</p>
            ...
            <form onSubmit={handleUpdatePo} className="space-y-4">
              <div>
                ...
                <input 
                  required value={formData.item}
                  className="w-full bg-slate-800 border border-slate-700 p-2 text-white outline-none focus:border-yellow-500"
                  onChange={e => setFormData({...formData, item: e.target.value})}
                />
              </div>
              <div>
              ...
              <div className="flex gap-3 pt-4">
                <button 
                  ...
                  className="flex-1 py-2 text-[10px] text-slate-500 hover:text-white"
                >
                  CANCEL
                </button>
                <button type="submit" className="flex-1 bg-yellow-600 py-2 text-[10px] font-bold text-black hover:bg-yellow-500">
                  UPDATE_DATA
                </button>
              </div>
            </form>
